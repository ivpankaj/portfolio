          ...
          <div className="w-full flex-1 lg:max-w-2xl">
            {lines.map((line, lineIndex) => {
              ...
              return (
                <span
                  key={lineIndex}
                  className="flex flex-wrap p-1 sm:p-2 text-4xl font-bold text-opacity-20 text-black dark:text-white dark:text-opacity-20"
                >
                  {words.map((word, i) => {
...
const Word: FC<WordProps> = ({ children, progress, range }) => {
  ...
  return (
    <span className="relative mx-0.5 sm:mx-1 lg:mx-2">
